/*
* This header is generated by classdump-dyld 1.0
* on Saturday, December 30, 2017 at 11:21:56 PM Japan Standard Time
* Operating System: Version 10.3.3 (Build 14G60)
* Image Source: /System/Library/AccessibilityBundles/DigitalTouchShared.axbundle/DigitalTouchShared
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <DigitalTouchShared/DigitalTouchShared-Structs.h>
#import <DigitalTouchShared/__ETCanvasViewAccessibility_super.h>

@interface ETCanvasViewAccessibility : __ETCanvasViewAccessibility_super
+(id)safeCategoryTargetClassName;
+(void)_accessibilityPerformValidations:(id)arg1 ;
+(Class)safeCategoryBaseClass;
-(void)_accessibilityFocusChanged:(id)arg1 ;
-(void)_accessibilityInvalidateDirectTouchTimerAndReschedule:(char)arg1 ;
-(void)_accessibilitySetIsDirectTouching:(char)arg1 ;
-(char)_accessibilityIsDirectTouching;
-(id)_accessibilityDisableDirectTouchTimer;
-(void)_accessibilitySetDisableDirectTouchTimer:(id)arg1 ;
-(id)initWithFrame:(CGRect)arg1 ;
-(void)dealloc;
-(void)touchesBegan:(id)arg1 withEvent:(id)arg2 ;
-(void)touchesEnded:(id)arg1 withEvent:(id)arg2 ;
-(CGRect)accessibilityFrame;
-(id)accessibilityLabel;
-(id)accessibilityHint;
-(unsigned long long)accessibilityTraits;
-(char)isAccessibilityElement;
-(char)accessibilityElementsHidden;
-(char)accessibilityActivate;
-(char)_accessibilityServesAsFirstElement;
-(void)_setIsComposingMessageOfType:(unsigned short)arg1 ;
-(void)_checkForIdle;
-(void)_endMessage:(id)arg1 withSend:(char)arg2 ;
@end

