/*
* This header is generated by classdump-dyld 1.0
* on Saturday, December 30, 2017 at 11:21:15 PM Japan Standard Time
* Operating System: Version 10.3.3 (Build 14G60)
* Image Source: /System/Library/Frameworks/UIKit.framework/UIKit
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <UIKit/UIKit-Structs.h>
#import <UIKit/UIView.h>
#import <UIKit/_UIIVCResponseDelegateImpl.h>
#import <UIKit/_UIKeyboardTextSelectionGestureControllerDelegate.h>
#import <UIKit/UITextInputSuggestionDelegate.h>
#import <UIKit/UIKeyboardCandidateListDelegate.h>

@protocol UIKeyInput, UIKeyboardCandidateList, UIKeyboardImplGeometryDelegate;
@class UIKeyboardTaskQueue, _UIKeyboardTextSelectionController, TIKeyboardState, TIKeyboardInputManagerState, UIView, NSString, UIKeyboardAutocorrectionController, UIAutocorrectInlinePrompt, TIKeyboardCandidateResultSet, UIKeyboardLayout, NSMutableDictionary, UIKeyboardScheduledTask, UIDelayedAction, UITextInputTraits, UILexicon, NSMutableArray, UITextInputArrowKeyHistory, _UIFeedbackKeyboardBehavior, TIKeyboardTouchEvent, _UIActionWhenIdle, UIPhysicalKeyboardEvent, UIAlertView, NSDictionary, UITextSelectionView, TIKeyboardInputManagerStub, UIResponder, TIKeyboardLayout;

@interface UIKeyboardImpl : UIView <_UIIVCResponseDelegateImpl, _UIKeyboardTextSelectionGestureControllerDelegate, UITextInputSuggestionDelegate, UIKeyboardCandidateListDelegate> {

	id<UIKeyInput> m_delegate;
	UIKeyboardTaskQueue* m_taskQueue;
	/*^block*/id m_externalTask;
	TIKeyboardState* m_keyboardState;
	TIKeyboardInputManagerState* m_inputManagerState;
	char m_keyboardDelegateStateNeedsRefresh;
	UIView* m_languageIndicator;
	NSString* m_previousInputString;
	UIKeyboardAutocorrectionController* m_autocorrectionController;
	UIAutocorrectInlinePrompt* m_autocorrectPrompt;
	TIKeyboardCandidateResultSet* m_candidateResultSet;
	id<UIKeyboardCandidateList> m_candidateList;
	UIView* m_markedTextOverlay;
	id<UIKeyboardImplGeometryDelegate> m_geometryDelegate;
	UIKeyboardLayout* m_layout;
	NSMutableDictionary* m_keyedLayouts;
	UIKeyboardScheduledTask* m_autoDeleteTask;
	unsigned m_autoDeleteCount;
	double m_autoDeleteLastDelete;
	double m_autoDeleteInterval;
	unsigned long m_autoDeleteShiftCharacter;
	float m_autoDeleteCurrentForceMultipler;
	float m_autoDeleteCurrentForce;
	char m_autoDeleteUseForce;
	UIDelayedAction* m_longPressAction;
	int m_originalOrientation;
	CGPoint m_inputPoint;
	int m_changeCount;
	double m_changeTime;
	id m_changedDelegate;
	CFRunLoopObserverRef m_observer;
	unsigned m_textInputChangingCount;
	char m_textInputChangingText;
	char m_textInputChangingDirection;
	char m_textInputChangesIgnored;
	char m_insideKeyInputDelegateCall;
	char m_disableSyncTextChanged;
	UITextInputTraits* m_defaultTraits;
	UITextInputTraits* m_traits;
	int m_returnKeyState;
	int m_currentDirection;
	UILexicon* m_recentInputs;
	char m_autoDeleteOK;
	char m_autocapitalizationPreference;
	char m_autocorrectionPreference;
	char m_autoshift;
	char m_caretShowingNow;
	char m_changeNotificationDisabled;
	char m_correctionLearningAllowed;
	char m_delegateIsSMSTextView;
	char m_delegateRequiresKeyEvents;
	char m_doubleSpacePeriodWasAppliedInCurrentContext;
	char m_doubleSpacePeriodPreference;
	char m_hardwareKeyboardAttached;
	char m_inDealloc;
	char m_initializationDone;
	char m_performanceLoggingEnabled;
	char m_selecting;
	char m_shift;
	char m_shiftLocked;
	char m_shiftLockedEnabled;
	char m_settingShift;
	char m_suggestionsShownForCurrentDeletion;
	char m_originalShouldSkipCandidateSelection;
	char m_updatingPreferences;
	char m_suppressGeometryChangeNotifications;
	char m_acceptingCandidate;
	char m_didAutomaticallyInsertSpace;
	char m_didSyncDocumentStateToInputDelegate;
	char m_userChangedSelection;
	char m_shouldChargeKeys;
	char m_longPress;
	char m_replacingWord;
	char m_shiftNeedsUpdate;
	char m_shiftPreventAutoshift;
	char m_shiftHeldDownNeedsUpdated;
	char m_delegateAdoptsTextInput;
	char m_delegateAdoptsTextInputPrivate;
	char m_delegateAdoptsWebTextInputPrivate;
	char m_delegateAdoptsKeyboardInput;
	char m_delegateAdoptsWKInteraction;
	char m_delegateAdoptsKeyInputPrivate;
	char m_clientVariantSupportEnabled;
	char m_clientVariantSupportEnabledEver;
	NSMutableArray* m_keyplaneNamesPreviousDelegate;
	NSMutableArray* m_keyplaneNamesCurrentDelegate;
	UITextInputArrowKeyHistory* m_arrowKeyHistory;
	char m_isRotating;
	char m_preRotateShift;
	char m_preRotateShiftLocked;
	char m_showInputModeIndicator;
	char m_suppressUpdateCandidateView;
	char m_shouldUpdateCacheOnInputModesChange;
	char m_rivenCenterFilled;
	float m_splitProgress;
	char m_needsCandidates;
	char m_shouldSkipCandidateGeneration;
	char m_updateLayoutOnShowKeyboard;
	char m_receivedCandidatesInCurrentInputMode;
	int _currentAlertReason;
	_UIFeedbackKeyboardBehavior* m_feedbackBehavior;
	char m_canUpdateIdleTimer;
	char m_scrolling;
	char m_hasInputOnAcceptCandidate;
	char m_maximizing;
	char m_usesAutoShiftFor3PK;
	unsigned m_previousSpaceKeyBehavior;
	unsigned m_previousReturnKeyBehavior;
	char m_usesCandidateSelection;
	char m_UsedCandidateSelection;
	char m_updatingLayout;
	char m_suppressUpdateLayout;
	char m_animateUpdateBars;
	char m_needsToRecomputeDesirableModes;
	char m_isCapsLocked;
	UITextInputTraits* m_traitsForPreviousDelegate;
	UIDelayedAction* m_detachHardwareKeyboardAction;
	UIView* m_capsLockSign;
	char m_didAutomaticallyInsertSpaceBeforeChangingInputMode;
	int m_predictionType;
	char m_repeatDeleteFromHardwareKeyboard;
	UIDelayedAction* m_disablePredictionViewTimer;
	double m_lastDisablePredictionViewTime;
	char m_showsCandidateBar;
	char m_showsCandidateInline;
	char committingCandidate;
	char geometryIsChanging;
	char m_softwareKeyboardShownByTouch;
	char _forceEnablePredictionView;
	char _handlingKeyCommandFromHardwareKeyboard;
	TIKeyboardTouchEvent* m_touchEventWaitingForKeyInputEvent;
	_UIActionWhenIdle* m_delayedCandidateRequest;
	_UIActionWhenIdle* m_deferredDidSetDelegateAction;
	UIPhysicalKeyboardEvent* m_hardwareRepeatEvent;
	UIKeyboardScheduledTask* m_hardwareRepeatTask;
	UIAlertView* keyboardAlertView;
	_UIKeyboardTextSelectionController* _textSelectionController;
	NSDictionary* _candidateRequestInfo;
	UIKeyboardScheduledTask* _autocorrectPromptTask;

}

@property (nonatomic,retain) id changedDelegate; 
@property (nonatomic,readonly) UIKeyboardAutocorrectionController * autocorrectionController; 
@property (nonatomic,readonly) UITextSelectionView * selectionView; 
@property (nonatomic,retain) UITextInputArrowKeyHistory * arrowKeyHistory; 
@property (nonatomic,readonly) TIKeyboardInputManagerStub * inputManager; 
@property (nonatomic,retain) TIKeyboardInputManagerState * inputManagerState; 
@property (nonatomic,copy) id externalTask; 
@property (nonatomic,retain) TIKeyboardTouchEvent * touchEventWaitingForKeyInputEvent; 
@property (nonatomic,retain) NSDictionary * candidateRequestInfo;                                                //@synthesize candidateRequestInfo=_candidateRequestInfo - In the implementation block
@property (nonatomic,retain) _UIActionWhenIdle * delayedCandidateRequest; 
@property (nonatomic,retain) UIKeyboardScheduledTask * autocorrectPromptTask;                                    //@synthesize autocorrectPromptTask=_autocorrectPromptTask - In the implementation block
@property (nonatomic,retain) _UIKeyboardTextSelectionController * textSelectionController;                       //@synthesize textSelectionController=_textSelectionController - In the implementation block
@property (nonatomic,retain) _UIActionWhenIdle * deferredDidSetDelegateAction; 
@property (nonatomic,retain) UIPhysicalKeyboardEvent * hardwareRepeatEvent; 
@property (nonatomic,retain) UIKeyboardScheduledTask * hardwareRepeatTask; 
@property (nonatomic,retain) UIAlertView * keyboardAlertView; 
@property (assign,nonatomic) char committingCandidate; 
@property (assign,nonatomic) char hardwareKeyboardIsSeen; 
@property (assign,nonatomic) char softwareKeyboardShownByTouch; 
@property (assign,nonatomic) char handlingKeyCommandFromHardwareKeyboard;                                        //@synthesize handlingKeyCommandFromHardwareKeyboard=_handlingKeyCommandFromHardwareKeyboard - In the implementation block
@property (assign,nonatomic) char showInputModeIndicator; 
@property (assign,nonatomic) char showsCandidateBar; 
@property (nonatomic,readonly) char shouldShowCandidateBar; 
@property (assign,nonatomic) char receivedCandidatesInCurrentInputMode; 
@property (assign,nonatomic) char showsCandidateInline; 
@property (assign,nonatomic) char geometryIsChanging; 
@property (assign,nonatomic) char shouldSkipCandidateSelection; 
@property (assign,nonatomic) char forceEnablePredictionView;                                                     //@synthesize forceEnablePredictionView=_forceEnablePredictionView - In the implementation block
@property (nonatomic,readonly) char isDefaultPrediction; 
@property (nonatomic,readonly) UIKeyboardTaskQueue * taskQueue; 
@property (assign,nonatomic) char suppressUpdateLayout; 
@property (assign,nonatomic) char animateUpdateBars; 
@property (readonly) char centerFilled; 
@property (assign) char rivenSplitLock; 
@property (readonly) unsigned minimumTouchesForTranslation; 
@property (nonatomic,readonly) char splitTransitionInProgress; 
@property (assign,getter=isInHardwareKeyboardMode,nonatomic) char inHardwareKeyboardMode; 
@property (assign,nonatomic) char canUpdateIdleTimer; 
@property (nonatomic,retain) UIResponder*<UIKeyInput> delegate; 
@property (assign,nonatomic) id<UIKeyboardImplGeometryDelegate> geometryDelegate; 
@property (nonatomic,readonly) UIResponder*<UITextInput> inputDelegate; 
@property (nonatomic,readonly) UIResponder*<UITextInputPrivate> privateInputDelegate; 
@property (nonatomic,readonly) id<UIKeyboardInput> legacyInputDelegate; 
@property (nonatomic,readonly) UIResponder*<UIWKInteractionViewProtocol> asynchronousInputDelegate; 
@property (nonatomic,readonly) UIResponder*<UIKeyInputPrivate> privateKeyInputDelegate; 
@property (nonatomic,readonly) UIResponder * delegateAsResponder; 
@property (nonatomic,retain) _UIFeedbackKeyboardBehavior * feedbackBehavior; 
@property (nonatomic,retain) TIKeyboardLayout * layoutForKeyHitTest; 
@property (readonly) unsigned hash; 
@property (readonly) Class superclass; 
@property (copy,readonly) NSString * description; 
@property (copy,readonly) NSString * debugDescription; 
@property (nonatomic,readonly) double timestampOfLastTouchesEnded; 
+(id)activeInstance;
+(id)sharedInstance;
+(id)keyboardScreen;
+(char)isSplit;
+(void)applicationDidReceiveMemoryWarning:(id)arg1 ;
+(void)applicationDidBecomeActive:(id)arg1 ;
+(void)applicationWillResignActive:(id)arg1 ;
+(void)applicationDidEnterBackground:(id)arg1 ;
+(void)applicationWillEnterForeground:(id)arg1 ;
+(void)applicationWillSuspend:(id)arg1 ;
+(void)purgeImageCache;
+(void)releaseSharedInstance;
+(void)setFloating:(char)arg1 ;
+(CGSize)sizeForInterfaceOrientation:(int)arg1 ;
+(char)rivenTranslationPreference;
+(void)setPersistentOffset:(CGPoint)arg1 ;
+(char)isFloating;
+(char)rivenPreference;
+(void)setPersistentSplitProgress:(float)arg1 ;
+(float)floatingWidth;
+(Class)layoutClassForInputMode:(id)arg1 keyboardType:(int)arg2 screenTraits:(id)arg3 ;
+(float)topMarginForInterfaceOrientation:(int)arg1 ;
+(CGSize)keyboardSizeForInterfaceOrientation:(int)arg1 ;
+(float)additionalInstanceHeightForInterfaceOrientation:(int)arg1 ;
+(CGSize)defaultSizeForInterfaceOrientation:(int)arg1 ;
+(char)keyboardOrientation:(int)arg1 isEquivalentToOrientation:(int)arg2 ;
+(Class)layoutClassForCurrentInputMode;
+(void)markPerformance:(id)arg1 ;
+(float)splitProgress;
+(void)markElapsed:(id)arg1 ;
+(void)suppressSetPersistentOffset:(char)arg1 ;
+(char)supportsSplit;
+(float)persistentSplitProgress;
+(void)refreshRivenStateWithTraits:(id)arg1 isKeyboard:(char)arg2 ;
+(char)supportsFloating;
+(CGPoint)persistentOffset;
+(CGPoint)normalizedPersistentOffset;
+(CGPoint)normalizedPersistentOffsetIgnoringState;
+(id)keyboardWindow;
+(char)isActivatingForeground;
+(float)additionalInstanceHeight;
+(void)sendPerformanceNotification:(id)arg1 ;
+(void)sendPerformanceNotification:(id)arg1 userInfo:(id)arg2 ;
+(id)uniqueNumberPadInputModesFromInputModes:(id)arg1 forKeyboardType:(int)arg2 ;
+(void)hardwareKeyboardAvailabilityChanged;
+(id)normalizedInputModesFromPreference;
+(char)rivenInstalled;
+(void)setFloating:(char)arg1 positionedWithLayout:(id)arg2 ;
+(float)keyboardWidthForScreen:(id)arg1 withOrientation:(int)arg2 ;
+(void)setParentTestForProfiling:(id)arg1 ;
+(void)_clearHardwareKeyboardMinimizationPreference;
-(UIKeyboardTaskQueue *)taskQueue;
-(void)_handleKeyEvent:(id)arg1 executionContext:(id)arg2 ;
-(void)removeFromSuperview;
-(void)deleteFromInput;
-(id)initWithFrame:(CGRect)arg1 ;
-(void)setFrame:(CGRect)arg1 ;
-(void)setShift:(char)arg1 ;
-(void)setDelegate:(UIResponder*<UIKeyInput>)arg1 ;
-(void)dealloc;
-(id)hitTest:(CGPoint)arg1 withEvent:(id)arg2 ;
-(char)pointInside:(CGPoint)arg1 withEvent:(id)arg2 ;
-(UIResponder*<UIKeyInput>)delegate;
-(char)pointInside:(CGPoint)arg1 forEvent:(GSEventRef)arg2 ;
-(void)cancelAllKeyEvents;
-(void)ejectKeyDown;
-(void)handleKeyEvent:(id)arg1 ;
-(char)handleKeyCommand:(id)arg1 repeatOkay:(char*)arg2 beforePublicKeyCommands:(char)arg3 ;
-(void)setCanUpdateIdleTimer:(char)arg1 ;
-(char)shouldAllowRepeatEvent:(id)arg1 ;
-(void)_wheelChangedWithEvent:(id)arg1 ;
-(char)isCapsLockASCIIToggle;
-(void)alertView:(id)arg1 clickedButtonAtIndex:(int)arg2 ;
-(int)changeCount;
-(void)_moveWithEvent:(id)arg1 ;
-(void)willMoveToWindow:(id)arg1 ;
-(void)didMoveToSuperview;
-(void)unmarkText;
-(UIResponder*<UITextInput>)inputDelegate;
-(void)insertText:(id)arg1 ;
-(void)deleteBackward;
-(int)returnKeyType;
-(_UIFeedbackKeyboardBehavior *)feedbackBehavior;
-(void)setFeedbackBehavior:(_UIFeedbackKeyboardBehavior *)arg1 ;
-(void)candidateListAcceptCandidate:(id)arg1 ;
-(void)candidateListSelectionDidChange:(id)arg1 ;
-(void)showPreviousCandidate;
-(void)showNextPage;
-(void)jumpToCompositions;
-(void)revealHiddenCandidates;
-(UITextSelectionView *)selectionView;
-(id)textInputTraits;
-(void)setSelectionWithPoint:(CGPoint)arg1 ;
-(void)textWillChange:(id)arg1 ;
-(void)textDidChange:(id)arg1 ;
-(char)hasMarkedText;
-(void)remoteControlReceivedWithEvent:(id)arg1 ;
-(void)detach;
-(void)updateLayout;
-(void)setCaretBlinks:(char)arg1 ;
-(void)layoutHasChanged;
-(void)setInitialDirection;
-(void)textFrameChanged:(id)arg1 ;
-(id)changedDelegate;
-(void)setChangedDelegate:(id)arg1 ;
-(void)textChanged:(id)arg1 ;
-(CGRect)subtractKeyboardFrameFromRect:(CGRect)arg1 inView:(id)arg2 ;
-(int)_clipCornersOfView:(id)arg1 ;
-(char)isLongPress;
-(char)splitTransitionInProgress;
-(char)rivenSplitLock;
-(void)setSplitProgress:(float)arg1 ;
-(void)clearAnimations;
-(void)removeAutocorrection;
-(void)logHandwritingData;
-(unsigned)maxNumberOfProactiveCells;
-(id)candidateList;
-(id)_layout;
-(char)supportsNumberKeySelection;
-(char)centerFilled;
-(char)isMinimized;
-(CGRect)dragGestureRectInView:(id)arg1 ;
-(void)releaseInputManager;
-(void)clearDelegate;
-(void)clearTimers;
-(void)clearLayouts;
-(char)usesCandidateSelection;
-(void)acceptAutocorrection;
-(void)clearTransientState;
-(void)deactivateLayout;
-(void)_updateSoundPreheatingForWindow:(id)arg1 ;
-(char)disableInputBars;
-(char)showsCandidateBar;
-(char)hardwareKeyboardIsSeen;
-(void)notifyShiftState;
-(void)setCapsLockSign;
-(void)takeTextInputTraitsFromDelegate;
-(void)setCapsLockIfNeeded;
-(char)shiftLockPreference;
-(char)doubleSpacePeriodPreference;
-(id)inputModePreference;
-(id)UILanguagePreference;
-(void)refreshRivenPreferences;
-(void)setInSplitKeyboardMode:(char)arg1 ;
-(void)setInputModeFromPreferences;
-(void)delayedInit;
-(void)applicationSuspendedEventsOnly:(id)arg1 ;
-(void)applicationResumedEventsOnly:(id)arg1 ;
-(void)keyboardDidHide:(id)arg1 ;
-(void)selectionScrolling:(id)arg1 ;
-(char)performanceLoggingPreference;
-(void)removeAutocorrectPromptAndCandidateList;
-(void)setInputMode:(id)arg1 ;
-(char)shouldShowCandidateBarIfReceivedCandidatesInCurrentInputMode:(char)arg1 ;
-(char)shouldSwitchFromInputManagerMode:(id)arg1 toInputMode:(id)arg2 ;
-(void)updateInputManagerMode;
-(id)layoutState;
-(char)candidateSelectionPredictionForTraits;
-(id)responderForSendCurrentLocation;
-(id)updateSecureCandidateRenderTraits;
-(UIKeyboardAutocorrectionController *)autocorrectionController;
-(void)clearInput;
-(UIAlertView *)keyboardAlertView;
-(void)setKeyboardAlertView:(UIAlertView *)arg1 ;
-(void)clearChangeTimeAndCount;
-(void)updateCandidateDisplay;
-(char)autocorrectionPreferenceForTraits;
-(void)setAutocorrectSpellingEnabled:(char)arg1 ;
-(char)autocapitalizationPreference;
-(void)updateInputManagerAutocapitalizationType;
-(void)recomputeActiveInputModesWithExtensions:(char)arg1 ;
-(void)syncInputManagerToKeyboardState;
-(void)toggleSoftwareKeyboard;
-(char)automaticMinimizationEnabled;
-(void)setSoftwareKeyboardShownByTouch:(char)arg1 ;
-(CGRect)frameForKeylayoutName:(id)arg1 ;
-(char)maximizing;
-(void)setRivenSplitLock:(char)arg1 ;
-(char)isInHardwareKeyboardMode;
-(char)autocorrectionPreference;
-(char)keyboardIsKeyPad;
-(char)forceEnablePredictionView;
-(char)predictionForTraitsWithForceEnable:(char)arg1 ;
-(char)predictionFromPreference;
-(char)predictionForTraits;
-(char)checkSpellingPreference;
-(void)setAutomaticMinimizationEnabled:(char)arg1 ;
-(void)setInputMode:(id)arg1 userInitiated:(char)arg2 ;
-(void)setInputMode:(id)arg1 userInitiated:(char)arg2 updateIndicator:(char)arg3 executionContext:(id)arg4 ;
-(void)setKeyboardInputMode:(id)arg1 userInitiated:(char)arg2 updateIndicator:(char)arg3 executionContext:(id)arg4 ;
-(void)setForceEnablePredictionView:(char)arg1 ;
-(void)cleanUpBeforeInputModeSwitch;
-(void)setInputManagerFromCurrentInputMode;
-(void)reinitializeAfterInputModeSwitch:(char)arg1 ;
-(void)updateShiftState;
-(void)setShouldSkipCandidateSelection:(char)arg1 ;
-(void)postInputViewControllerShouldUpdateNotification:(id)arg1 ;
-(void)finishSetKeyboardInputMode:(id)arg1 didChangeDirection:(char)arg2 ;
-(void)textChanged:(id)arg1 executionContext:(id)arg2 ;
-(void)syncInputManagerToKeyboardStateWithExecutionContext:(id)arg1 ;
-(char)animateUpdateBars;
-(void)setShowsCandidateBar:(char)arg1 ;
-(char)shouldShowCandidateBar;
-(void)generateCandidates;
-(void)callLayoutUpdateLocalizedKeys;
-(void)updatePredictionView;
-(void)updateInputModeIndicatorOnSingleKeyOnly:(char)arg1 ;
-(void)finishSetExtensionInputMode:(id)arg1 didChangeDirection:(char)arg2 ;
-(void)finishSetInputMode:(id)arg1 didChangeDirection:(char)arg2 ;
-(void)showKeyboard;
-(void)hideKeyboard;
-(void)updateTextCandidateView;
-(void)setKeyboardInputModeFromPreferences:(id)arg1 ;
-(void)prepareKeyboardInputModeFromPreferences:(id)arg1 ;
-(void)setKeyboardInputMode:(id)arg1 userInitiated:(char)arg2 ;
-(void)clearLanguageIndicator;
-(void)fadeAnimationDidStop:(id)arg1 finished:(id)arg2 ;
-(void)updateInputModeIndicatorOnSingleKeyOnly:(char)arg1 preserveIfPossible:(char)arg2 ;
-(void)showInformationalAlertIfNeededForReason:(int)arg1 ;
-(void)finishSetInputModeToNextInPreferredListWithExecutionContext:(id)arg1 ;
-(void)updateInputModeLastChosenPreference;
-(void)showInternationalKeyInfoAlertIfNeeded;
-(char)showInputModeIndicator;
-(void)updateLayoutIfNecessary;
-(void)prepareForGeometryChange;
-(void)setShiftLocked:(char)arg1 ;
-(void)updateStylingTraitsIfNeeded;
-(void)geometryChangeDone:(char)arg1 ;
-(UIResponder*<UITextInputPrivate>)privateInputDelegate;
-(char)shouldUseCarPlayModes;
-(UIResponder*<UIWKInteractionViewProtocol>)asynchronousInputDelegate;
-(void)setTextSelectionController:(_UIKeyboardTextSelectionController *)arg1 ;
-(void)setDelegate:(id)arg1 force:(char)arg2 ;
-(char)suppressUpdateLayout;
-(void)setSuppressUpdateLayout:(char)arg1 ;
-(void)updateReturnKey:(char)arg1 ;
-(void)setAnimateUpdateBars:(char)arg1 ;
-(void)setCaretVisible:(char)arg1 ;
-(void)storeDelegateConformance;
-(void)setTextSelectionControllerFromDelegate;
-(void)stopAutoDelete;
-(void)clearShiftState;
-(void)callChanged;
-(char)delegateIsSMSTextView;
-(void)setArrowKeyHistory:(UITextInputArrowKeyHistory *)arg1 ;
-(void)setReturnKeyEnabled:(char)arg1 ;
-(id)desirableInputModesWithExtensions:(char)arg1 ;
-(void)callLayoutUpdateAllLocalizedKeys;
-(void)_updateExternalDeviceInputSettingForWindow:(id)arg1 ;
-(_UIActionWhenIdle *)deferredDidSetDelegateAction;
-(void)didSetDelegate;
-(void)setDeferredDidSetDelegateAction:(_UIActionWhenIdle *)arg1 ;
-(void)updateForChangedSelection;
-(void)updateForChangedSelectionWithExecutionContext:(id)arg1 ;
-(void)releaseInputManagerIfInactive;
-(char)checkSpellingPreferenceForTraits;
-(void)insertTextSuggestion:(id)arg1 ;
-(char)isAutoFillMode;
-(id)markedText;
-(void)clearSelection;
-(void)selectionWillChange:(id)arg1 ;
-(void)finishTextChanged;
-(void)updateNoContentViews;
-(void)setChanged;
-(void)setShiftOffIfNeeded;
-(_UIKeyboardTextSelectionController *)textSelectionController;
-(void)selectionDidChange:(id)arg1 ;
-(void)clearDisablePredictionViewTimer;
-(char)predictionPreferenceForTraits;
-(void)disablePredictionViewIfNeeded;
-(void)handlePredictionViewIfNeeded;
-(id)searchStringForMarkedText;
-(void)setCandidates:(id)arg1 ;
-(void)setCandidateRequestInfo:(NSDictionary *)arg1 ;
-(void)updateCandidateDisplayAsyncWithCandidateSet:(id)arg1 ;
-(char)callShouldReplaceExtendedRange:(int)arg1 withText:(id)arg2 includeMarkedText:(char)arg3 ;
-(char)callShouldInsertText:(id)arg1 ;
-(char)_shouldRequestInputManagerSyncForKeyboardOutputCallbacks:(id)arg1 ;
-(void)collapseSelectionAndAdjustByOffset:(int)arg1 ;
-(void)_updateInputViewControllerOutput:(id)arg1 forKeyboardOutput:(id)arg2 ;
-(void)performKeyboardOutput:(id)arg1 ;
-(void)_performInputViewControllerOutput:(id)arg1 ;
-(void)_handleWebKeyEvent:(id)arg1 withInputString:(id)arg2 executionContext:(id)arg3 ;
-(void)_processInputViewControllerKeyboardOutput:(id)arg1 executionContext:(id)arg2 ;
-(void)_completePerformInputViewControllerOutput:(id)arg1 executionContext:(id)arg2 ;
-(UIResponder *)delegateAsResponder;
-(void)updateHardwareKeyboardLayout;
-(void)setInputModeToNextInPreferredListWithExecutionContext:(id)arg1 ;
-(void)dismissKeyboard;
-(void)_requestInputManagerSync;
-(id<UIKeyboardInput>)legacyInputDelegate;
-(TIKeyboardInputManagerState *)inputManagerState;
-(char)usesAutoDeleteWord;
-(char)shouldRapidDeleteWithDelegate;
-(void)syncDocumentStateToInputDelegate;
-(void)updateAutocorrectPrompt:(id)arg1 ;
-(void)updateObserverState;
-(UITextInputArrowKeyHistory *)arrowKeyHistory;
-(void)setHandlingKeyCommandFromHardwareKeyboard:(char)arg1 ;
-(char)hasEditableMarkedText;
-(id)inputForMarkedText;
-(void)movePhraseBoundaryToDirection:(int)arg1 granularity:(int)arg2 ;
-(char)cursorIsAtEndOfMarkedText;
-(void)handleClear;
-(void)deleteForwardAndNotify:(char)arg1 ;
-(char)handleTabWithShift:(char)arg1 beforePublicKeyCommands:(char)arg2 ;
-(void)fadeAutocorrectPrompt;
-(id)_keyboardBehaviorState;
-(void)performKeyBehaviorConfirm;
-(void)performKeyBehaviorConfirmFirstCandidate;
-(void)setGeometryIsChanging:(char)arg1 ;
-(void)updateLayoutIfNeeded;
-(void)removeAutocorrectPrompt;
-(void)releaseSuppressUpdateCandidateView;
-(void)clearLongPressTimer;
-(char)canShowAppConnections;
-(char)isUsingDictationLayout;
-(id)dynamicCaretList;
-(char)showsCandidateInline;
-(char)currentKeyboardTraitsAllowCandidateBar;
-(void)_didChangeKeyplaneWithContext:(id)arg1 ;
-(void)callLayoutUpdateReturnKey;
-(void)finishLayoutChangeWithArguments:(id)arg1 ;
-(double)timestampOfLastTouchesEnded;
-(char)shouldAllowSelectionGestures:(char)arg1 atPoint:(CGPoint)arg2 toBegin:(char)arg3 ;
-(void)setTwoFingerTapTimestamp:(double)arg1 ;
-(CGSize)stretchFactor;
-(id)inputStringFromPhraseBoundary;
-(void)clearInputForMarkedText;
-(void)syncDocumentStateToInputDelegateWithExecutionContext:(id)arg1 ;
-(TIKeyboardInputManagerStub *)inputManager;
-(void)prepareForSelectionChange;
-(void)setPreviousInputString:(id)arg1 ;
-(char)shouldGenerateCandidatesAfterSelectionChange;
-(void)completeUpdateForChangedSelection:(char)arg1 ;
-(char)canOfferPredictionsForTraits;
-(void)updateReturnKey;
-(void)callChangedSelection;
-(void)deleteHandwritingStrokesAtIndexes:(id)arg1 ;
-(void)flushTouchEventWaitingForKeyInputEventIfNecessary;
-(void)refreshKeyboardState;
-(void)syncKeyboardToConfiguration:(id)arg1 ;
-(void)setInputManagerState:(TIKeyboardInputManagerState *)arg1 ;
-(void)setMarkedText:(id)arg1 selectedRange:(NSRange)arg2 inputString:(id)arg3 searchString:(id)arg4 ;
-(char)isShifted;
-(char)isShiftLocked;
-(void)setLabel:(id)arg1 forKey:(id)arg2 ;
-(void)cancelCandidateRequests;
-(TIKeyboardTouchEvent *)touchEventWaitingForKeyInputEvent;
-(void)setTouchEventWaitingForKeyInputEvent:(TIKeyboardTouchEvent *)arg1 ;
-(char)shouldApplyAcceptedAutocorrection:(id)arg1 ;
-(void)acceptCandidate:(id)arg1 ;
-(void)unmarkText:(id)arg1 ;
-(void)insertTextAfterSelection:(id)arg1 ;
-(void)deleteBackwardAndNotify:(char)arg1 ;
-(void)setAutocorrection:(id)arg1 ;
-(id)autocorrectPrompt;
-(void)touchAutocorrectPromptTimer;
-(void)performSendCurrentLocation;
-(void)generateCandidatesAsynchronouslyWithRange:(NSRange)arg1 selectedCandidate:(id)arg2 ;
-(NSDictionary *)candidateRequestInfo;
-(void)inputManagerDidGenerateCandidatesForRequest:(id)arg1 resultSet:(id)arg2 ;
-(_UIActionWhenIdle *)delayedCandidateRequest;
-(void)setDelayedCandidateRequest:(_UIActionWhenIdle *)arg1 ;
-(void)inputManagerDidGenerateAutocorrections:(id)arg1 executionContext:(id)arg2 ;
-(void)generateAutocorrectionsWithCompletionHandler:(/*^block*/id)arg1 ;
-(void)setAutocorrectionList:(id)arg1 ;
-(void)updateAutocorrectPrompt:(id)arg1 executionContext:(id)arg2 ;
-(void)moveCursorByAmount:(int)arg1 ;
-(void)skipHitTestForTouchEvent:(id)arg1 ;
-(void)didChangePhraseBoundary;
-(char)shouldSwitchInputMode:(id)arg1 ;
-(void)callLayoutSetShift:(char)arg1 ;
-(void)callLayoutSetAutoshift:(char)arg1 ;
-(char)usesAutocorrectionLists;
-(char)callLayoutIsShiftKeyPlaneChooser;
-(char)callLayoutIsShiftKeyBeingHeld;
-(char)callLayoutUsesAutoShift;
-(char)isSelectionAtSentenceAutoshiftBoundary;
-(void)setShiftPreventAutoshift:(char)arg1 ;
-(void)setShift:(char)arg1 autoshift:(char)arg2 ;
-(void)setShiftNeedsUpdate;
-(char)isCapsLockSwitchEnabled;
-(char)shouldSkipCandidateSelection;
-(void)setShouldSkipCandidateSelection:(char)arg1 userInitiated:(char)arg2 ;
-(char)suppliesCompletions;
-(char)shouldRapidDelete;
-(float)scaleCurrentForceFrom:(float)arg1 to:(float)arg2 ;
-(void)deleteFromInputWithFlags:(unsigned)arg1 executionContext:(id)arg2 ;
-(void)handleDeleteAsRepeat:(char)arg1 executionContext:(id)arg2 ;
-(void)handleDeleteWithExecutionContext:(id)arg1 ;
-(void)handleClearWithExecutionContext:(id)arg1 ;
-(void)addInputString:(id)arg1 withFlags:(unsigned)arg2 withInputManagerHint:(id)arg3 executionContext:(id)arg4 ;
-(char)acceptInputString:(id)arg1 ;
-(UIResponder*<UIKeyInputPrivate>)privateKeyInputDelegate;
-(void)updateKeyboardStateForInsertion:(id)arg1 ;
-(void)updateKeyboardStateForDeletion;
-(void)addInputString:(id)arg1 withFlags:(unsigned)arg2 ;
-(char)callLayoutIgnoresShiftState;
-(void)updateDoubleSpacePeriodStateForCharacter:(unsigned long)arg1 ;
-(char)shouldDeleteAutospaceBeforeTerminator:(id)arg1 ;
-(char)applyAutocorrection:(id)arg1 ;
-(void)addInputString:(id)arg1 withFlags:(unsigned)arg2 executionContext:(id)arg3 ;
-(void)setExternalTask:(id)arg1 ;
-(void)removeSpaceAfterEmojiForInputString:(id)arg1 ;
-(id)inputEventForInputString:(id)arg1 ;
-(void)addInputEvent:(id)arg1 executionContext:(id)arg2 ;
-(void)acceptCurrentCandidateIfSelectedWithExecutionContext:(id)arg1 ;
-(void)updateChangeTimeAndIncrementCount;
-(void)collapseSelection;
-(void)acceptCurrentCandidateForInput:(id)arg1 ;
-(void)addWordTerminator:(id)arg1 afterSpace:(char)arg2 elapsedTime:(double)arg3 executionContext:(id)arg4 ;
-(void)completeAddInputString:(id)arg1 ;
-(void)acceptAutocorrectionForWordTerminator:(id)arg1 executionContextPassingTIKeyboardCandidate:(id)arg2 ;
-(void)willReplaceTextInRangedSelectionWithKeyboardInput;
-(void)completeAddInputString:(id)arg1 generateCandidates:(char)arg2 ;
-(void)handleKeyboardInput:(id)arg1 executionContext:(id)arg2 ;
-(char)shouldAcceptCandidate:(id)arg1 beforeInputString:(id)arg2 ;
-(void)didAcceptAutocorrection:(id)arg1 wordTerminator:(id)arg2 ;
-(void)acceptAutocorrection:(id)arg1 executionContextPassingTIKeyboardCandidate:(id)arg2 ;
-(id)inputWordForTerminatorAtSelection;
-(char)nextCharacterIsWordCharacter;
-(void)syncInputManagerToAcceptedAutocorrection:(id)arg1 forInput:(id)arg2 ;
-(void)moveSelectionToEndOfWord;
-(void)setDocumentStateForAutocorrection:(id)arg1 ;
-(void)animateAutocorrectionToRect:(CGRect)arg1 fromRect:(CGRect)arg2 ;
-(void)didApplyAutocorrection:(id)arg1 autocorrectPromptFrame:(CGRect)arg2 ;
-(void)animateAutocorrectionToText:(id)arg1 fromRect:(CGRect)arg2 ;
-(void)trackUsageForAcceptedAutocorrection:(id)arg1 promptWasShowing:(char)arg2 ;
-(id)handleDeleteAutospaceForInputString:(id)arg1 afterSpace:(char)arg2 ;
-(id)handleDoubleSpacePeriodForInputString:(id)arg1 afterSpace:(char)arg2 elapsedTime:(double)arg3 ;
-(id)handleReplacement:(id)arg1 forSpaceAndInput:(id)arg2 ;
-(void)updateDoubleSpacePeriodStateForString:(id)arg1 ;
-(void)setUserSelectedCurrentCandidate:(char)arg1 ;
-(unsigned)phraseBoundary;
-(void)adjustPhraseBoundaryInForwardDirection:(char)arg1 granularity:(int)arg2 executionContext:(id)arg3 ;
-(void)completeDeleteOnceFromInputWithCharacterBefore:(unsigned long)arg1 ;
-(void)scheduleReplacementsWithOptions:(unsigned)arg1 ;
-(char)selectionIsEndOfWord;
-(void)scheduleReplacementsAfterDeletionToEndOfWord;
-(char)shouldEnableShiftForDeletedCharacter:(unsigned long)arg1 ;
-(void)deleteFromInputWithFlags:(unsigned)arg1 ;
-(char)callShouldDeleteWithWordCountForRapidDelete:(int)arg1 characterCountForRapidDelete:(int)arg2 ;
-(void)deleteOnceFromInputWithExecutionContext:(id)arg1 ;
-(void)completeDeleteFromInput;
-(char)userSelectedCurrentCandidate;
-(id)acceptAutocorrectionForWordTerminator:(id)arg1 ;
-(void)acceptCurrentCandidate;
-(id)emojiCandidate:(id)arg1 ;
-(void)acceptPredictiveInput:(id)arg1 executionContext:(id)arg2 ;
-(void)acceptCandidate:(id)arg1 forInput:(id)arg2 ;
-(char)committingCandidate;
-(void)setCommittingCandidate:(char)arg1 ;
-(char)acceptWord:(id)arg1 firstDelete:(unsigned)arg2 forInput:(id)arg3 ;
-(void)handleDeletionForCandidate:(id)arg1 ;
-(void)acceptCandidate:(id)arg1 forInput:(id)arg2 executionContext:(id)arg3 ;
-(void)completeAcceptCandidate:(id)arg1 ;
-(void)handleAcceptedCandidate:(id)arg1 executionContext:(id)arg2 ;
-(char)geometryIsChanging;
-(void)generateCandidatesWithOptions:(int)arg1 ;
-(void)trackUsageForCandidateAcceptedAction:(id)arg1 ;
-(void)updateKeyboardConfigurations;
-(void)addInputObject:(id)arg1 executionContext:(id)arg2 ;
-(void)removeCandidateList;
-(void)completeAcceptCandidateBeforeAddingInputObject:(id)arg1 executionContext:(id)arg2 ;
-(char)acceptWord:(id)arg1 firstDelete:(unsigned)arg2 ;
-(void)generateCandidatesAsynchronously;
-(void)_setNeedsCandidates:(char)arg1 ;
-(char)needsToDeferUpdateTextCandidateView;
-(UIKeyboardScheduledTask *)autocorrectPromptTask;
-(void)setAutocorrectPromptTask:(UIKeyboardScheduledTask *)arg1 ;
-(void)resizeCandidateBarWithDelta:(float)arg1 ;
-(void)clearAutocorrectPromptTimer;
-(void)sendCallbacksForPostCorrectionsRemoval;
-(char)noContent;
-(void)_nop;
-(void)showNextCandidates;
-(id)returnKeyDisplayName;
-(char)handlingKeyCommandFromHardwareKeyboard;
-(void)acceptFirstCandidate;
-(id)updateKeyBehaviors:(id)arg1 withBehaviors:(id)arg2 forState:(id)arg3 ;
-(void)_handleKeyBehavior:(unsigned)arg1 forKeyType:(id)arg2 ;
-(id)updatedKeyBehaviors;
-(id)inputOverlayContainer;
-(char)delegateSupportsCorrectionUI;
-(CGRect)convertRectToAutocorrectRect:(CGRect)arg1 delegateView:(id)arg2 container:(id)arg3 ;
-(id)autocorrectPromptRectsFromFirstDelegateRect:(CGRect)arg1 lastDelegateRect:(CGRect)arg2 ;
-(void)updateAutocorrectPrompt:(id)arg1 correctionRects:(id)arg2 ;
-(id)autocorrectPromptRectsForInput:(id)arg1 ;
-(id)_rangeForAutocorrectionText:(id)arg1 ;
-(void)trackUsageForPromptedCorrection:(id)arg1 inputString:(id)arg2 previousPrompt:(id)arg3 ;
-(void)sendCallbacksForPreCorrectionsDisplay;
-(void)autocorrectionAnimationDidStop:(id)arg1 finished:(id)arg2 context:(id)arg3 ;
-(char)delegateSuggestionsForCurrentInput;
-(id)autocorrectionRecordForWord:(id)arg1 ;
-(id)replacementsFromSelectedText;
-(void)generateReplacementsForString:(id)arg1 candidatesHandler:(/*^block*/id)arg2 executionContext:(id)arg3 ;
-(void)clearAutoDeleteTimer;
-(void)touchAutoDeleteTimerWithThreshold:(double)arg1 adjustForPartialCompletion:(char)arg2 ;
-(void)handleAutoDeleteWithExecutionContext:(id)arg1 ;
-(void)completeHandleAutoDelete;
-(double)timeoutForCurrentForce;
-(char)canUpdateIdleTimer;
-(float)convertForceToGrams:(float)arg1 ;
-(void)startAutoDeleteTimer;
-(void)handleDelayedActionLongPress:(id)arg1 ;
-(void)touchLongPressTimerWithDelay:(double)arg1 userInfo:(id)arg2 ;
-(void)touchLongPressTimerWithDelay:(double)arg1 ;
-(double)shouldExtendLongPressAction:(id)arg1 ;
-(void)longPressAction:(id)arg1 ;
-(void)callLayoutLongPressAction;
-(void)clearDetachHardwareKeyboardAction;
-(void)hardwareKeyboardAvailabilityChanged;
-(void)setInHardwareKeyboardMode:(char)arg1 ;
-(void)detachHardwareKeyboard;
-(UIKeyboardScheduledTask *)hardwareRepeatTask;
-(UIPhysicalKeyboardEvent *)hardwareRepeatEvent;
-(void)handleKeyEvent:(id)arg1 executionContext:(id)arg2 ;
-(void)setHardwareRepeatTask:(UIKeyboardScheduledTask *)arg1 ;
-(void)setHardwareRepeatEvent:(UIPhysicalKeyboardEvent *)arg1 ;
-(void)_handleWebKeyEvent:(id)arg1 withIndex:(unsigned)arg2 inInputString:(id)arg3 executionContext:(id)arg4 ;
-(id)externalTask;
-(void)_handleWebKeyEvent:(id)arg1 withEventType:(int)arg2 withInputString:(id)arg3 withInputStringIgnoringModifiers:(id)arg4 executionContext:(id)arg5 ;
-(void)handleModifiersChangeForKeyEvent:(id)arg1 executionContext:(id)arg2 ;
-(void)updateKeyboardEventsLagging:(id)arg1 ;
-(void)setHardwareKeyboardIsSeen:(char)arg1 ;
-(char)softwareKeyboardShownByTouch;
-(void)_remapKeyEvent:(id)arg1 withKeyEventMap:(id)arg2 ;
-(char)handleKeyCommand:(id)arg1 repeatOkay:(char*)arg2 ;
-(char)isValidKeyInput:(id)arg1 ;
-(void)completeHandleKeyEvent:(id)arg1 ;
-(void)handleKeyWithString:(id)arg1 forKeyEvent:(id)arg2 executionContext:(id)arg3 ;
-(void)handleStringInput:(id)arg1 withFlags:(unsigned)arg2 withInputManagerHint:(id)arg3 executionContext:(id)arg4 ;
-(char)canHandleEvent:(id)arg1 ;
-(char)caretBlinks;
-(char)canHandleKeyHitTest;
-(char)transitionInProgress;
-(void)setSuggestions:(id)arg1 ;
-(void)setCandidateList:(id)arg1 ;
-(void)candidateListInvalidateSelection:(id)arg1 ;
-(TIKeyboardLayout *)layoutForKeyHitTest;
-(void)setLayoutForKeyHitTest:(TIKeyboardLayout *)arg1 ;
-(void)defaultsDidChange;
-(unsigned)minimumTouchesForTranslation;
-(char)hideAccessoryViewsDuringSplit;
-(void)cancelSplitTransition;
-(char)swipeToTabPreference;
-(char)keyboardsExpandedPreference;
-(void)saveInputModesPreference:(id)arg1 ;
-(id)inputModeLastUsedPreference;
-(void)setShouldUpdateCacheOnInputModesChange:(char)arg1 ;
-(id)hardwareKeyboardsSeenPreference;
-(void)setHardwareKeyboardsSeenPreference:(id)arg1 ;
-(void)setInputModeToNextASCIICapableInPreferredList;
-(void)prepareLayoutForInterfaceOrientation:(int)arg1 ;
-(void)updateLayoutToCurrentInterfaceOrientation;
-(void)finishLayoutToCurrentInterfaceOrientation;
-(void)setCorrectionLearningAllowed:(char)arg1 ;
-(char)autocorrectSpellingEnabled;
-(void)setDefaultTextInputTraits:(id)arg1 ;
-(void)enable;
-(char)isDefaultPrediction;
-(char)changeNotificationDisabled;
-(void)setChangeNotificationDisabled:(char)arg1 ;
-(void)handleObserverCallback;
-(id)markedTextOverlay;
-(void)generateAutocorrectionWithExecutionContext:(id)arg1 ;
-(void)skipHitTestForTouchEvent:(id)arg1 delayed:(char)arg2 ;
-(void)performHitTestForTouchEvent:(id)arg1 executionContextPassingNSNumber:(id)arg2 ;
-(void)removeAllDynamicDictionaries;
-(void)toggleShift;
-(void)setShiftLockedForced:(char)arg1 ;
-(char)isAutoShifted;
-(char)shiftLockedEnabled;
-(void)forceShiftUpdate;
-(void)forceShiftUpdateIfKeyboardStateChanged;
-(void)setUsesCandidateSelection:(char)arg1 ;
-(void)keyActivated;
-(void)keyDeactivated;
-(void)setInputPoint:(CGPoint)arg1 ;
-(void)handleDelete;
-(void)handleClearWithInsertBeforeAdvance:(id)arg1 ;
-(char)handleTabWithShift:(char)arg1 ;
-(void)addInputString:(id)arg1 ;
-(void)addInputString:(id)arg1 fromVariantKey:(char)arg2 ;
-(void)addInputString:(id)arg1 withFlags:(unsigned)arg2 withInputManagerHint:(id)arg3 ;
-(void)updateLayoutAndSetShift;
-(void)didHandleWebKeyEvent;
-(void)acceptPredictiveInput:(id)arg1 ;
-(void)rejectAutocorrection:(id)arg1 ;
-(void)acceptCurrentCandidateIfSelected;
-(void)clearRecentInput;
-(void)removeRecentInput:(id)arg1 completionHandler:(/*^block*/id)arg2 ;
-(void)acceptRecentInput:(id)arg1 ;
-(void)addInputObject:(id)arg1 ;
-(void)replaceText:(id)arg1 ;
-(char)displaysCandidates;
-(void)setShowsCandidateInline:(char)arg1 ;
-(void)responseContextDidChange;
-(char)returnKeyEnabled;
-(void)_updateKeyboardConfigurations;
-(void)generateCandidatesFromReplacements;
-(id)generateAutocorrectionReplacements:(id)arg1 ;
-(id)generateAutocorrectionListForSelectedText;
-(void)startAutoDeleteTimerForForce:(float)arg1 ;
-(void)adjustAutoDeleteTimerForForce:(float)arg1 ;
-(void)touchLongPressTimer;
-(void)longPressAction;
-(void)firstHardwareAutoRepeatWithExecutionContext:(id)arg1 ;
-(void)showKeyboardIfNeeded;
-(void)startCaretBlinkIfNeeded;
-(char)caretVisible;
-(id<UIKeyboardImplGeometryDelegate>)geometryDelegate;
-(void)setGeometryDelegate:(id<UIKeyboardImplGeometryDelegate>)arg1 ;
-(void)setShowInputModeIndicator:(char)arg1 ;
-(char)receivedCandidatesInCurrentInputMode;
-(void)setReceivedCandidatesInCurrentInputMode:(char)arg1 ;
-(void)callLayoutUpdateRecentInputs;
-(void)timeMark:(unsigned)arg1 message:(id)arg2 ;
-(void)timeMark:(unsigned)arg1 ;
-(void)timeElapsed:(unsigned)arg1 message:(id)arg2 ;
-(void)_setInputManager:(id)arg1 ;
-(void)_setShiftLockedEnabled:(char)arg1 ;
-(char)hasAutoRepeat;
-(void)updateFromTextInputTraits;
-(char)_isShowingCandidateUIWithAvailableCandidates;
-(int)textInputChangingCount;
-(void)testAutocorrectionPromptWithCorrection:(id)arg1 ;
-(char)isAutoDeleteActive;
-(void)composeTypologyRadar;
-(void)createTypoTrackerReport;
-(id)textInteractionAssistant;
-(void)setSplit:(char)arg1 animated:(char)arg2 ;
-(void)finishSplitTransitionWithProgress:(float)arg1 ;
-(void)clearExcessKeyboardMemory;
-(char)keyboardDrawsOpaque;
-(char)_shouldMinimizeForHardwareKeyboard;
-(void)setTextInputChangesIgnored:(char)arg1 ;
-(void)acceptAutocorrectionAndEndComposition;
-(void)moveCursorLeftShifted:(char)arg1 ;
-(void)moveCursorRightShifted:(char)arg1 ;
-(void)lookUpResponderChainForCustomInputView;
-(char)_activeCandidateViewNeedsBackdrop;
-(id)_getCurrentKeyplaneName;
-(id)_getCurrentKeyboardName;
-(id)_getLocalizedInputMode;
-(void)_setAutocorrects:(char)arg1 ;
-(id)_getAutocorrection;
-(char)_hasCandidates;
-(int)_positionInCandidateList:(id)arg1 ;
-(char)_needsCandidates;
@end

