/*
* This header is generated by classdump-dyld 1.0
* on Friday, December 29, 2017 at 6:29:51 PM Japan Standard Time
* Operating System: Version 10.3.3 (Build 14G60)
* Image Source: /System/Library/Frameworks/UIKit.framework/UIKit
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <UIKit/UIKit-Structs.h>
#import <UIKit/UIView.h>

@protocol UITextInput;
@class UITextSelectionView, UIResponder, NSArray, NSMutableArray, UITouch, UIView, UISelectionGrabber;

@interface UITextRangeView : UIView {

	UITextSelectionView* m_selectionView;
	UIResponder*<UITextInput> m_container;
	int m_mode;
	NSArray* m_rects;
	NSMutableArray* m_rectViews;
	UITouch* m_activeTouch;
	CGRect m_startEdge;
	CGRect m_endEdge;
	CGPoint m_basePoint;
	CGPoint m_extentPoint;
	CGPoint m_initialBasePoint;
	CGPoint m_initialExtentPoint;
	float m_initialDistance;
	CGPoint m_touchOffset;
	double m_firstMovedTime;
	UIView* m_rectContainerView;
	UISelectionGrabber* m_startGrabber;
	UISelectionGrabber* m_endGrabber;
	char m_animateUpdate;
	char m_baseIsStart;
	char m_commandsWereShowing;
	char m_inGesture;
	char m_magnifying;
	char m_scrolling;
	char m_scaling;
	char m_rotating;
	char m_inputViewIsChanging;
	char m_isClearingRange;
	char m_shouldStayVisible;

}

@property (assign,nonatomic) CGRect startEdge; 
@property (assign,nonatomic) CGRect endEdge; 
@property (assign,nonatomic) CGPoint basePoint; 
@property (assign,nonatomic) CGPoint extentPoint; 
@property (assign,nonatomic) CGPoint initialBasePoint; 
@property (assign,nonatomic) float initialDistance; 
@property (assign,nonatomic) CGPoint touchOffset; 
@property (assign,nonatomic) double firstMovedTime; 
@property (assign,nonatomic) char magnifying; 
@property (assign,nonatomic) char isClearingRange; 
@property (assign,nonatomic) char shouldStayVisible; 
@property (nonatomic,retain) UITouch * activeTouch; 
@property (nonatomic,readonly) UITextSelectionView * selectionView; 
@property (nonatomic,readonly) UIResponder*<UITextInput> container; 
@property (assign,nonatomic) int mode; 
@property (nonatomic,retain) NSArray * rects; 
@property (nonatomic,readonly) char autoscrolled; 
@property (assign,nonatomic) char inGesture; 
@property (assign,nonatomic) char isScrolling; 
@property (assign,nonatomic) char rotating; 
@property (assign,nonatomic) char scaling; 
@property (assign,nonatomic) char inputViewIsChanging; 
@property (assign,nonatomic) char baseIsStart; 
@property (assign,nonatomic) char commandsWereShowing; 
@property (nonatomic,retain) UISelectionGrabber * startGrabber; 
@property (nonatomic,retain) UISelectionGrabber * endGrabber; 
@property (assign,nonatomic) CGPoint initialExtentPoint; 
@property (assign,nonatomic) char animateUpdate; 
@property (nonatomic,readonly) CGPoint activeTouchPoint; 
-(void)removeFromSuperview;
-(char)pointInside:(CGPoint)arg1 withEvent:(id)arg2 ;
-(void)touchesBegan:(id)arg1 withEvent:(id)arg2 ;
-(void)touchesMoved:(id)arg1 withEvent:(id)arg2 ;
-(void)touchesEnded:(id)arg1 withEvent:(id)arg2 ;
-(void)touchesCancelled:(id)arg1 withEvent:(id)arg2 ;
-(UIResponder*<UITextInput>)container;
-(UITextSelectionView *)selectionView;
-(CGRect)_selectionClipRect;
-(void)updateSelectionWithDocumentPoint:(CGPoint)arg1 ;
-(void)stopAnimating;
-(void)startAnimating;
-(void)setMode:(int)arg1 ;
-(int)mode;
-(void)willRotate;
-(void)didRotate;
-(void)setInGesture:(char)arg1 ;
-(void)setActiveTouch:(UITouch *)arg1 ;
-(UITouch *)activeTouch;
-(char)inGesture;
-(char)autoscrolled;
-(void)updateBaseIsStartWithDocumentPoint:(CGPoint)arg1 ;
-(char)baseIsStart;
-(void)setBaseIsStart:(char)arg1 ;
-(void)setAnimateUpdate:(char)arg1 ;
-(UISelectionGrabber *)startGrabber;
-(UISelectionGrabber *)endGrabber;
-(void)updateBaseAndExtentPointsFromEdges;
-(CGPoint)extentPoint;
-(CGPoint)activeTouchPoint;
-(CGPoint)applyTouchOffset:(CGPoint)arg1 ;
-(void)beginMagnifying;
-(char)commandsWereShowing;
-(CGPoint)magnifierPoint;
-(CGPoint)initialExtentPoint;
-(void)willScroll;
-(void)didScroll;
-(void)inputViewWillChange;
-(void)inputViewDidChange;
-(void)clearRangeAnimated:(char)arg1 ;
-(void)setRects:(NSArray *)arg1 ;
-(void)updateDots;
-(id)initWithFrame:(CGRect)arg1 selectionView:(id)arg2 ;
-(NSArray *)rects;
-(void)prepareForMagnification;
-(void)doneMagnifying;
-(void)scaleWillChange;
-(void)scaleDidChange;
-(void)cancelDelayedActions;
-(void)setIsScrolling:(char)arg1 ;
-(CGRect)startEdge;
-(CGRect)endEdge;
-(char)_endIsHorizontal;
-(char)_startIsHorizontal;
-(void)setCommandsWereShowing:(char)arg1 ;
-(void)updateEdges;
-(char)pointCloserToEnd:(CGPoint)arg1 startEdge:(CGRect)arg2 endEdge:(CGRect)arg3 ;
-(CGPoint)basePoint;
-(void)setInitialBasePoint:(CGPoint)arg1 ;
-(void)setInitialExtentPoint:(CGPoint)arg1 ;
-(CGPoint)initialBasePoint;
-(void)setInitialDistance:(float)arg1 ;
-(void)setTouchOffset:(CGPoint)arg1 touchPoint:(CGPoint)arg2 ;
-(void)setFirstMovedTime:(double)arg1 ;
-(void)setExtentPoint:(CGPoint)arg1 ;
-(double)firstMovedTime;
-(float)initialDistance;
-(void)updateRectViews;
-(void)updateGrabbers;
-(void)setStartEdge:(CGRect)arg1 ;
-(void)setEndEdge:(CGRect)arg1 ;
-(char)isClearingRange;
-(void)setShouldStayVisible:(char)arg1 ;
-(void)setMagnifying:(char)arg1 ;
-(void)setScaling:(char)arg1 ;
-(void)setRotating:(char)arg1 ;
-(void)setInputViewIsChanging:(char)arg1 ;
-(void)setIsClearingRange:(char)arg1 ;
-(char)shouldStayVisible;
-(void)setTouchOffset:(CGPoint)arg1 ;
-(CGPoint)touchOffset;
-(void)setMagnifierOrientation;
-(void)setBasePoint:(CGPoint)arg1 ;
-(CGPoint)convertFromMagnifierPoint:(CGPoint)arg1 ;
-(void)setStartGrabber:(UISelectionGrabber *)arg1 ;
-(void)setEndGrabber:(UISelectionGrabber *)arg1 ;
-(char)animateUpdate;
-(char)magnifying;
-(char)isScrolling;
-(char)scaling;
-(char)rotating;
-(char)inputViewIsChanging;
@end

