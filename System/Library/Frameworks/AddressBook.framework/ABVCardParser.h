/*
* This header is generated by classdump-dyld 1.0
* on Saturday, December 30, 2017 at 11:21:39 PM Japan Standard Time
* Operating System: Version 10.3.3 (Build 14G60)
* Image Source: /System/Library/Frameworks/AddressBook.framework/AddressBook
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/


#import <AddressBook/AddressBook-Structs.h>
@class ABVCardValueSetter, ABVCardLexer, NSData, NSString, NSMutableArray, NSMutableDictionary, NSMutableString, ABVCardWatchdogTimer;

@interface ABVCardParser : NSObject {

	ABVCardValueSetter* _valueSetter;
	void* _source;
	ABVCardLexer* _lexer;
	NSData* _data;
	unsigned _defaultEncoding;
	char _hasImportErrors;
	char _30vCard;
	NSString* _first;
	NSString* _last;
	NSString* _org;
	NSString* _formattedName;
	NSMutableArray* _emails;
	NSMutableArray* _dates;
	NSMutableArray* _phones;
	NSMutableArray* _addresses;
	NSMutableArray* _aims;
	NSMutableArray* _jabbers;
	NSMutableArray* _msns;
	NSMutableArray* _yahoos;
	NSMutableArray* _icqs;
	NSMutableArray* _untypedIMs;
	NSMutableArray* _instantMessengers;
	NSMutableArray* _socialProfiles;
	NSMutableArray* _relatedNames;
	NSMutableArray* _urls;
	NSMutableDictionary* _activityAlerts;
	NSMutableString* _notes;
	NSMutableString* _otherNotes;
	NSMutableDictionary* _extensions;
	NSString* _uid;
	NSData* _imageData;
	NSString* _imageURI;
	int _cropRectX;
	int _cropRectY;
	int _cropRectWidth;
	int _cropRectHeight;
	NSData* _cropRectChecksum;
	NSMutableArray* _itemParameters;
	NSString* _grouping;
	unsigned _encoding;
	char _quotedPrintable;
	char _base64;
	ABVCardWatchdogTimer* _timer;

}

@property (assign,nonatomic) void* source; 
+(CFArrayRef)supportedProperties;
-(void)dealloc;
-(void)setSource:(void*)arg1 ;
-(id)initWithData:(id)arg1 ;
-(void*)source;
-(char)_usesArrayForExternalPropKey:(id)arg1 ;
-(id)initWithData:(id)arg1 watchdogTimer:(id)arg2 ;
-(void)cleanUpCardState;
-(id)defaultLabel;
-(void)setLocalRecordHasAdditionalProperties:(char)arg1 ;
-(void)_setValueFromExtension:(id)arg1 forKey:(id)arg2 onAddress:(id)arg3 toKey:(id)arg4 ;
-(int)_addIMPPProfiles:(id)arg1 multiValue:(void*)arg2 uniquenessCheckingMultiValue:(void*)arg3 ;
-(int)_addIMHandles:(id)arg1 toService:(CFStringRef)arg2 multiValue:(void*)arg3 uniquenessCheckingMultiValue:(void*)arg4 ;
-(char)_setPersonSounds:(void*)arg1 identifier:(int)arg2 fromActivity:(id)arg3 alert:(id)arg4 otherValue:(id)arg5 ;
-(void)noteLackOfValueForProperty:(unsigned)arg1 ;
-(void)addMultiValues:(id)arg1 toProperty:(unsigned)arg2 valueComparator:(/*^block*/id)arg3 ;
-(char)_setStringValueOrNoteIfNull:(id)arg1 forProperty:(unsigned)arg2 ;
-(id)_genericLabel;
-(id)parseSingleValue;
-(id)genericLabel;
-(id)phoneLabel;
-(id)_socialProfileService;
-(id)_socialProfileUsername;
-(id)_socialProfileUserId;
-(id)_socialProfileDisplayName;
-(id)_socialProfileTeamIdentifier;
-(id)_socialProfileBundleIdentifiers;
-(id)parseInstantMessengerProfile:(id)arg1 ;
-(id)parseRemainingLine;
-(char)parseUID;
-(char)parseTEL;
-(char)parseADR;
-(char)parseORG;
-(id)parseURL;
-(id)defaultURLLabel;
-(char)parseADD;
-(char)parseEMAIL;
-(char)addIMValueTo:(id)arg1 ;
-(char)parsePhoto:(id)arg1 ;
-(char)parseN;
-(char)parseVERSION;
-(char)parseABUID;
-(char)parseABExtensionType:(id)arg1 ;
-(char)parseBDAY;
-(char)parseIMPP;
-(char)parseNICKNAME;
-(char)parseABDATE;
-(void)parseABOrder;
-(void)parseABPhoto;
-(char)parseAlternateBirthday;
-(void)parseABShowAs;
-(char)parseABMaiden;
-(char)parseSocialProfiles;
-(char)parseABReleatedNames;
-(char)parseActivityAlerts;
-(id)parseValueArray;
-(char)_usesRemainingLineForExternalPropKey:(id)arg1 ;
-(char)_handleUnknownTag:(id)arg1 withValue:(id)arg2 ;
-(char)importToValueSetter:(id)arg1 ;
-(char)parseItem;
-(char)_setMultiValuesOrNoteIfNull:(id)arg1 forProperty:(unsigned)arg2 valueComparator:(/*^block*/id)arg3 ;
-(void)addAddressMultiValues;
-(void)addSocialProfileMultiValues;
-(void)addInstantMessageMultiValues;
-(void)addActivityAlertMultiValues;
-(void)noteLackOfValueForImageData;
-(char)importToPerson:(void*)arg1 foundProperties:(const _CFArray*)arg2 ;
-(void*)copyNextPersonWithLength:(int*)arg1 foundProperties:(const _CFArray*)arg2 ;
-(char)hasImportErrors;
-(CFArrayRef)peopleAndProperties:(const _CFArray*)arg1 ;
-(char)_setIntValueOrNoteIfNull:(int)arg1 forProperty:(int)arg2 ;
-(char)_setDataValueOrNoteIfNull:(id)arg1 forProperty:(unsigned)arg2 ;
-(id)defaultADRLabel;
-(id)_valueSetter;
-(id)sortedPeopleAndProperties:(const _CFArray*)arg1 ;
-(id)dateFromISO8601String:(id)arg1 ;
-(void)finalize;
@end

