/*
* This header is generated by classdump-dyld 1.0
* on Friday, December 29, 2017 at 6:38:08 PM Japan Standard Time
* Operating System: Version 10.3.3 (Build 14G60)
* Image Source: /Applications/Diagnostics.app/Diagnostics
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <Diagnostics/Diagnostics-Structs.h>
#import <Diagnostics/Diagnostics.BaseView.h>

@class DACheckAnimationView, UIView;

@interface Diagnostics.CheckmarkView : Diagnostics.BaseView {

	 yOffset;
	 padding;
	 checkAnimationView;
	 circleView;

}

@property (readonly,nonatomic) DACheckAnimationView * checkAnimationView; 
@property (readonly,nonatomic) UIView * circleView; 
-(void)setupViewAttributes;
-(void)setupViewHierarchy;
-(void)setupViewLayoutConstraints;
-(DACheckAnimationView *)checkAnimationView;
-(id)initWithFrame:(CGRect)arg1 ;
-(id)initWithCoder:(id)arg1 ;
-(UIView *)circleView;
@end

